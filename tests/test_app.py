...
# Must be set BEFORE `from app import ...` runs, since app/__init__.py reads
...
from app import app, mydb, TimelinePost, NAV_PAGES
...
class AppTestCase(unittest.TestCase):
    ...
    # Helper, not auto-run in setUp (unlike test_db.py): not every test here
    # wants the same starting posts -- test_get_posts_empty specifically
    # needs the table to start empty. Tests opt in by calling this
    # explicitly when they DO want seeded data. Only appropriate for tests
    # that need existing rows as setup -- NOT for tests that are actually
    # verifying the POST route creates rows correctly (those must go
    # through self.client.post(...), or they'd stop testing the route).
    ...
    def test_timeline_page_loads(self):
        ...
        self.assertIn('<h1 class="page-heading">Timeline</h1>', html)
        self.assertIn('id="timeline-form"', html)
        self.assertIn('id="timeline-posts"', html)
        self.assertIn('id="timeline-message"', html)

    # GET /api/timeline_post

    def test_get_posts_empty(self):
        # setUp clears the table before every test, so this should always start from zero 
        # this tests 1st thing a fresh install of app sees before anyone posts anything (makes sure empty [] returns, not null or crash)
...
# Sends a real HTTP POST (through self.client.post(...), exactly like a browser would), and checks the JSON response looks right.
# Then does a separate GET request afterward, and checks the post actually landed in the database.
        ...
        # Confirm it actually persisted, not just in response.
...
# checks status is 400
# checks that response.is.json is true (disinguishes graceful JSON validation error vs. Flask's default crash page, which also happens to return 400)
    ...
    def test_post_rejects_blank_field(self):
        # "content" is present but empty -- distinct edge case from a missing key entirely, since request.form.get() alone wouldn't
        # catch this without the .strip() + truthiness check.
        response = self.client.post(
            "/api/timeline_post",
    ...
    def test_post_rejects_invalid_email(self):
        # Very basic check: a valid-looking email needs at least an "@"
        # and a "." -- not full RFC-grade validation, just enough to catch
        # obvious typos/garbage input.
        response = self.client.post(
            "/api/timeline_post",
    ...
    def test_delete_success(self):
        ...
        # call) also lets this test confirm DELETE only removes the targeted row -- Jane should still be there afterward.
        john, jane = self._seed_john_and_jane()

        response = self.client.delete(f"/api/timeline_post/{john.id}")

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json(), {"deleted": john.id})
...
# allows us to run this file like a normal python script in the terminal, i.e. "python test_app.py"
